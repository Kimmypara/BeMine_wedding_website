...
<span class="endpoint-box1">/vendor/create.php</span>
...
The request worked, so a new Vendor was created.
...
```json
{
    "message": "Vendor not created. Missing or invalid input."
}
```
...
Only administrators can create vendors.
...
```json
{
    "message": "Vendor not created. Vendor already exists."
}
```
...
The server failed to create the Vendor.
